...
_OLLAMA_MODEL_FOR_TYPE = {
    "DMPC":    "llava",             # Formulaire structuré â†’ llava rapide suffit
    "PVa":     "llama3.2-vision",   # Texte libre tapé â†’ meilleure compréhension sémantique
...
def _validate_field(field_type: str, val: str, commune_db=None) -> Optional[str]:
    ...
    LABEL_INTERDITS = [
        ...
        "procàƒ¨s-verbal", "document d'arpentage", "bornage", "plan de", "feuille", "n°", "numero", "date", "d",
    ]
...
def _verify_and_revise(champs, all_ocr_page, img_shape, full_text, commune_db=None):
    ...
    # â•â• PASSE B : Itération sur les champs incertains (jusqu'à  3 tentatives) â•â•
    ...
    if uncertain_fields:
        print(f"\n  [Verify] â•â• Passe B : Relance itérative ({len(uncertain_fields)} champs suspects) â•â•")
        STRATEGIES = [
    ...
    # â•â• PASSE C : Cohérence inter-champs â•â•
    print(f"\n  [Verify] â•â• Passe C : Cohérence inter-champs â•â•")
    ...
    print(f"  [Verify] â•â• Vérification terminée â•â•\n")
    return verified
...
def classify_plan(filepath: str) -> str:
    ...
    # 2. Dàƒ©tection depuis le nom de fichier
...
def _extract_dmpc_specialized(
    ...
    # â”€â”€ Rà‰ACTIVATION DU NOM DE FICHIER POUR LES DOCUMENTS ILLISIBLES â”€â”€
    ...
    # â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    # -- Niveau 1 : Nom de fichier (Dà‰SACTIVà‰ À LA DEMANDE DE L'UTILISATEUR) --
    # if pdf_path:
...
def process_plan(pdf_path: str, models=None, commune_db=None) -> dict:
    ...
    for pi, page in enumerate(doc):
        ...
        print(f"  [PlanClassifier] Page {page_num}/{nb_pages} ââ‚¬â€ ràƒ´le: [{role}]")
        ...
        for field, zone in zones_def.items():
                ...
                if result and result.get('methode') != 'graph_inline' and result.get('methode') != 'graph_neighbor':
                    print(f"    [{field}] ââ€ â€™ '{result['valeur']}' (Contextuel)")
                ...
                if not result and field in KEYWORDS and KEYWORDS[field]:
                    ...
                    if result:
                        print(f"    [{field}] ââ€ â€™ '{result['valeur']}' (Mots-clàƒ©s)")
                ...
                if not result and _GLINER_AVAILABLE:
                    ...
                    if result:
                        print(f"    [{field}] ââ€ â€™ '{result['valeur']}' (GLiNER)")
        ...
        if type_plan == "DMPC":
            ...
            if "geometre" not in champs:
                ...
                for nom_connu in GEOMETRES_CONNUS:
                    if nom_connu.lower() in full_text_page_low:
                        ...
                        print(f"    [geometre] ââ€ â€™ '{nom_connu}' (Fallback Global VIP)")
                        break
            
            # ââ€â‚¬ââ€â‚¬ Fallback Global Infaillible pour Section et Feuille ââ€â‚¬ââ€â‚¬
            if "section" not in champs:
                ...
                if m_sec:
                    ...
                    print(f"    [section] ââ€ â€™ '{m_sec.group(1).upper()}' (Fallback Global)")
            if "feuille" not in champs:
                m_feu = re.search(r'(?i)\bfeuille\s+(?:n[oà‚°])?\s*(\d{1,3})\b', full_text_page)
                if m_feu:
                    champs["feuille"] = {
                    ...
                    print(f"    [feuille] ââ€ â€™ '{m_feu.group(1)}' (Fallback Global)")
        ...
        if all_missing and type_plan in ["CROQUIS", "PVa", "GENERIC"]:
        ...
        if _champs_a_recontroler:
            ...
            for field in sorted(_champs_a_recontroler):
                ...
                if _valeurs_blacklist and field in ("proprietaires_anciens", "proprietaires_nouveaux", "signataires", "geometre"):
                    ...
                    if candidats_filtres:
                        if len(candidats_filtres) < len(candidats):
                            ...
                            print(f"    [Vote][{field}] ðŸš« Blacklist élimine : {evicted}")
                ...
                if val_existante and val_choisie.lower() == val_existante.lower():
                    if nb_methodes > 1:
                        print(f"    [Vote][{field}] âœ“ '{val_choisie}' confirmé ({nb_methodes}/{nb_total} méthodes)")
                        ...
                        print(f"    [Vote][{field}] âš  '{val_choisie}' trouvé par 1 méthode seulement")
                else:
                    if accord or conf_finale > (champs.get(field) or {}).get("confidence", 0):
                        print(f"    [Vote][{field}] â†º '{val_existante}' â†’ '{val_choisie}' ({nb_methodes} méthodes, {conf_finale:.0%})")
                        champs[field] = {
                            **(meilleur.get("raw", {})),
                            "valeur": val_choisie,
